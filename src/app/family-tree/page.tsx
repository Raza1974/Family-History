    ...
    <div className="family-tree p-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2 print:grid-cols-3">
        {familyData.map((member) => (
          <div
            key={member.id}
            className={`family-member border p-2 m-1 rounded-lg shadow-md text-sm ${
          ...
          >
            ...
            <div className="mt-1 space-y-1 print:hidden">
              <Link href={`/edit-member/${member.id}`} className="block text-center bg-blue-500 text-white py-1 px-2 rounded text-xs hover:bg-blue-600">
                Edit
              </Link>
              <button onClick={() => handleDelete(member.id)} className="w-full bg-red-500 text-white py-1 px-2 rounded text-xs hover:bg-red-600">
                Delete
              </button>
              <button onClick={() => handleAddChild(member.id)} className="w-full bg-green-500 text-white py-1 px-2 rounded text-xs hover:bg-green-600">
                Add Child
              </button>
              <button onClick={() => handleAddSibling(member.id)} className="w-full bg-yellow-500 text-white py-1 px-2 rounded text-xs hover:bg-yellow-600">
                Add Sibling
              </button>
              <button onClick={() => handleAddSpouse(member.id)} className="w-full bg-purple-500 text-white py-1 px-2 rounded text-xs hover:bg-purple-600">
                Add Spouse
              </button>
            </div>
          </div>
      ...
      <Link href="/" className="block mt-4 text-center bg-gray-500 text-white py-2 px-4 rounded hover:bg-gray-600 cursor-pointer print:hidden">
        Back to Home
      </Link>
    </div>
  );
